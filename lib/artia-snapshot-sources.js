...
async function buildSnapshotFromUpstream() {
  ...
  try {
    ...
    const workGroupField = readEnv("ARTIA_WORK_GROUP_FIELD", "workGroup");
    ...
    return buildPayload(
      rows.map((row) => ({
        ...
        workGroup: getByPath(row, workGroupField),
