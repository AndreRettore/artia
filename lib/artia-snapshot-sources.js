...
function getDefaultDbQuery() {
  ...
  const activitiesTable = readEnv("ARTIA_DB_ACTIVITIES_TABLE", `organization_${orgId}_activities_v2`);
...
async function buildSnapshotFromDb() {
  const sql =
    readEnv("ARTIA_DB_QUERY") ||
    (readEnv("ARTIA_DB_QUERY_MODE").toLowerCase() === "time_entries"
